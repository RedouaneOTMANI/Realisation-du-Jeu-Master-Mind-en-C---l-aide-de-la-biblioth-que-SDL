...
#include "Score.h"
#include"DUP.h"
/**Choix du code a l'aide de la fonction RAND **/
...
//on effectue notre comparaison dans cette fonction
void eval_codeJeuNiveau1 (int code[4], int prop[4], int res[2]) {
	...
	for (i = 0; i < 4; i++) {
		if (prop[i] == code[i]) {
			res[0]++;//les deux pions Rouge et blanche
			st_prop[i] = 1;
			st_code[i] = 1;
		}
	}
...
void init_pos_resJeuNiveau1 (SDL_Rect pos[2], int n) {
	int i;
	for (i = 0; i < 2; i++) {
		pos[i].x = 450 + 40 * i;
		pos[i].y = 380 - 35 * n;
...
/**La position du 10 lignes d'essais */
...
/**la position des bottons annuler et valider */
...
for (i = 2; i < 4; i++) {
	...
	init_pos_optJeuNiveau1(pos_opt, 0);
	init_pos_resJeuNiveau1(pos_res, 0);
	...
	while (continuer == 1) {
        ...
        switch (event.type) {
            ...
            case SDL_MOUSEMOTION:
            	...
            	if (!over) {
            		if (is_overJeuNiveau1(x_m, y_m, *annul, pos_opt[0])) {
            			SDL_BlitSurface(vide, NULL, ecran, &pos_opt[0]);
            		} else if (all_fillJeuNiveau1(tab[n][0]) && is_overJeuNiveau1(x_m, y_m, *valid, pos_opt[1])) {
            			SDL_BlitSurface(vide, NULL, ecran, &pos_opt[1]);
            		} else if (all_fillJeuNiveau1(tab[n][0])) {
            	...
            	if (is_overJeuNiveau1(x_m, y_m, *jouer, pos_jou)) {
            		if (!over) {
            			SDL_BlitSurface(abandons, NULL, ecran, &pos_jou);
            		} else {
            			SDL_BlitSurface(jouers, NULL, ecran, &pos_jou);
            		}
            	} else if (is_overJeuNiveau1(x_m, y_m, *menu, pos_men)) {
            		SDL_BlitSurface(menus, NULL, ecran, &pos_men);
            	} else if (!over && is_overJeuNiveau1(x_m, y_m, *aide, pos_reg)) {
            ...
            case SDL_MOUSEBUTTONUP:
		        x_m = event.button.x;
		       	...
		       	if (over && is_overJeuNiveau1(x_m, y_m, *jouer, pos_jou)) {
            		continuer = 3;
            		pass = 1;
            	} else if (is_overJeuNiveau1(x_m, y_m, *menu, pos_men)) {
            		continuer = 2;
		        ...
		        /* Bouton 'valider' */
		        if (!pass && !over) {
		        	if (all_fillJeuNiveau1(tab[n][0]) && is_overJeuNiveau1(x_m, y_m, *valid, pos_opt[1])) {
		        		SDL_BlitSurface(none, NULL, ecran, &pos_opt[0]);
		        		SDL_BlitSurface(none, NULL, ecran, &pos_opt[1]);
		        		SDL_BlitSurface(none, NULL, ecran, &pos_opt[2]);
		        		eval_codeJeuNiveau1(code, tab[n][0], tab[n][1]);
						if (n == 9 || tab[n][1][0] == 4) {
		        			for (i = 0; i < 4; i++) {
								pos_chx[i].x = 230 + 50 * i;
								pos_chx[i].y = 17;
							...
							init_pos_resJeuNiveau1(pos_res, n);
							init_pos_optJeuNiveau1(pos_opt, n);
