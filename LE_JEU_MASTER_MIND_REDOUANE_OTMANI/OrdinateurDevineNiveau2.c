...
void init_pos_evaOrdinateurDevineNiveau2 (SDL_Rect pos[3], int n) {
	int i;
	for (i = 0; i < 3; i++) {
		pos[i].x = 460 + 70 * i;
		pos[i].y = 378 - 35 * n;
...
int ready_togoOrdinateurDevineNiveau2 (int code[4], int n_past[2], int n, int sens) { /* 0 = haut, 1 = bas */
	if (sens == 0) {
		if (all_fillJeuNiveau2(code) && n_past[0] + n_past[1] <= 4 && n < 9) {
			return 1;
		}
		return 0;
	}
...
int nb_remainOrdinateurDevineNiveau2 (int * tab[10][2], int n, int * poss) {
	...
	for (i = 0; i < pow(8, 4); i++) {
		poss[i] = 1;
		for (j = 0; j < n; j++) {
			decodeOrdinateurDevineNiveau2(i, blank4);
			eval_codeJeuNiveau2(blank4, tab[j][0], blank2);
...
void assistOrdinateurDevineNiveau2 (int * tab[10][2], int n, int * poss, int s, int conseil[4]) {
	if (s > 0) {
		...
		decodeOrdinateurDevineNiveau2(k, conseil);
	} else {
		conseil[0] = -1;
	}
}
...
int i, j;
    ...
    for(i = 6; i < 8; i++) {posp[i].x = -319 + 60 * i;
		posp[i].y = 380;
		SDL_SetColorKey(pions[i], SDL_SRCCOLORKEY, SDL_MapRGB(pions[i]->format, 68, 116, 213));
		SDL_BlitSurface(pions[i], NULL, ecran, &posp[i]);
	}
	...
	init_pos_fleOrdinateurDevineNiveau2(pos_fle, 0);
	init_pos_evaOrdinateurDevineNiveau2(pos_eva, 0);
	init_pos_evoOrdinateurDevineNiveau2(pos_evo, 0);
	init_pos_nbOrdinateurDevineNiveau2(pos_nb, 0);
	...
	while (continuer == 1) {
        ...
        switch (event.type) {
            ...
            case SDL_MOUSEMOTION:
            	...
            	if (is_overJeuNiveau2(x_m, y_m, *menu, pos_men)) {
            		SDL_BlitSurface(menus, NULL, ecran, &pos_men);
            	}  else if (is_overJeuNiveau2(x_m, y_m, *aide, pos_reg)) {
            		SDL_BlitSurface(aides, NULL, ecran, &pos_reg);
            		SDL_BlitSurface(aide_g, NULL, ecran, &pos_aide);
            	} else if (ready_togoOrdinateurDevineNiveau2(tab[n][0], tab[n][1], n, 0) && is_overJeuNiveau2(x_m, y_m, *haut, pos_evo[0])) {
            		SDL_BlitSurface(hauts, NULL, ecran, &pos_evo[0]);
            	} else if (ready_togoOrdinateurDevineNiveau2(tab[n][0], tab[n][1], n, 1) && is_overJeuNiveau2(x_m, y_m, *haut, pos_evo[1])) {
            		SDL_BlitSurface(bass, NULL, ecran, &pos_evo[1]);
            	} else if (is_overJeuNiveau2(x_m, y_m, *help, pos_eva[2])) {
            		...
            		if (ready_togoOrdinateurDevineNiveau2(tab[n][0], tab[n][1], n, 0)) {
            			SDL_BlitSurface(haut, NULL, ecran, &pos_evo[0]);
            		}
            		if (ready_togoOrdinateurDevineNiveau2(tab[n][0], tab[n][1], n, 1)) {
            ...
            case SDL_MOUSEBUTTONUP:
		        ...
		        Conseil */
		        if (!pass) {
		        	if (is_overJeuNiveau2(x_m, y_m, *help, pos_eva[2])) {
		        		assistOrdinateurDevineNiveau2(tab, n, poss, s_possible, blank4);
						if (blank4[0] != -1) {
							for (i = 0; i < 4; i++) {
		        ...
		        if (!pass) {
		        	if (is_overJeuNiveau2(x_m, y_m, *p_droite, pos_fle[1]) && tab[n][1][0] < 4) {
		        		...
		        		pass = 1;
		        	} else if (is_overJeuNiveau2(x_m, y_m, *p_gauche, pos_fle[0]) && tab[n][1][0] > 0) {
		        		SDL_BlitSurface(p_rouge[--tab[n][1][0]], NULL, ecran, &pos_eva[0]);
		        		SDL_BlitSurface(p_droite, NULL, ecran, &pos_fle[1]);
		        		if (tab[n][1][0] == 0) {
		        		...
		        		pass = 1;
		        	} else if (is_overJeuNiveau2(x_m, y_m, *p_droite, pos_fle[3]) && tab[n][1][1] < 4) {
		        		...
		        		pass = 1;
		        	} else if (is_overJeuNiveau2(x_m, y_m, *p_gauche, pos_fle[2]) && tab[n][1][1] > 0) {
		        		SDL_BlitSurface(p_blanc[--tab[n][1][1]], NULL, ecran, &pos_eva[1]);
		        		SDL_BlitSurface(p_droite, NULL, ecran, &pos_fle[3]);
		        		if (tab[n][1][1] == 0) {
		        ...
		        /* Flèches d'évolution */
			    if (!pass) {
			    	if (ready_togoOrdinateurDevineNiveau2(tab[n][0], tab[n][1], n, 0) && is_overJeuNiveau2(x_m, y_m, *haut, pos_evo[0])) {
			    		chgt = 1;
			    	} else if (ready_togoOrdinateurDevineNiveau2(tab[n][0], tab[n][1], n, 1) && is_overJeuNiveau2(x_m, y_m, *bas, pos_evo[1])) {
			    		chgt = -1;
			    	}
			    	...
			    	if (chgt != 0) {
						...
						n += chgt;
						s_possible = nb_remainOrdinateurDevineNiveau2 (tab, n, poss);

						if (chgt > 0) {
							nb_decompoOrdinateurDevineNiveau2(s_possible, blank4);
							...
							init_pos_nbOrdinateurDevineNiveau2(pos_nb, n);
						...
						init_pos_fleOrdinateurDevineNiveau2(pos_fle, n);
						init_pos_evaOrdinateurDevineNiveau2(pos_eva, n);
						init_pos_evoOrdinateurDevineNiveau2(pos_evo, n);
